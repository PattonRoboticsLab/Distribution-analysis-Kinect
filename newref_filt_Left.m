function [newref_filt_wrist,newref_filt_elbow,newref_filt_shoulder,newref_filt_hip,newref_filt_spine,left_wrist,left_wrist_filt,left_elbow,left_elbow_filt,left_shoulder,left_shoulder_filt,L,time,tot_time]=newref_filt_Left(activity,b,a,theta1,theta2)
...
loc_shoulder=5;     %left shoulder
loc_elbow=6;       %left elbow
loc_wrist=7;       %left wrist
loc_hip=13;         %left hip
loc_spine=21;       %spine shoulder

for i=1:L
    left_wrist(i,:)=bodyparts((L*(loc_wrist-1))+i,:);
    left_elbow(i,:)=bodyparts((L*(loc_elbow-1))+i,:);
    left_shoulder(i,:)=bodyparts((L*(loc_shoulder-1))+i,:);
    left_hip(i,:)=bodyparts((L*(loc_hip-1))+i,:);
    spinesh(i,:)=bodyparts((L*(loc_spine-1))+i,:);
end
 
%% Filtering without changing refsystem 

left_wrist_filt(:,1)=filtfilt(b,a,left_wrist(:,1));
left_wrist_filt(:,2)=filtfilt(b,a,left_wrist(:,2));
left_wrist_filt(:,3)=filtfilt(b,a,left_wrist(:,3));

left_elbow_filt(:,1)=filtfilt(b,a,left_elbow(:,1));
left_elbow_filt(:,2)=filtfilt(b,a,left_elbow(:,2));
left_elbow_filt(:,3)=filtfilt(b,a,left_elbow(:,3));

left_shoulder_filt(:,1)=filtfilt(b,a,left_shoulder(:,1));
left_shoulder_filt(:,2)=filtfilt(b,a,left_shoulder(:,2));
left_shoulder_filt(:,3)=filtfilt(b,a,left_shoulder(:,3));

left_hip_filt(:,1)=filtfilt(b,a,left_hip(:,1));
left_hip_filt(:,2)=filtfilt(b,a,left_hip(:,2));
left_hip_filt(:,3)=filtfilt(b,a,left_hip(:,3));
...
for i=1:length(left_wrist(:,1))      % same of elbow, shoulder, hip
    newposition=rot1*left_wrist_filt(i,:)'; left_wrist_rot1(i,:)=newposition';
    newposition=rot1*left_elbow_filt(i,:)'; left_elbow_rot1(i,:)=newposition';
    newposition=rot1*left_shoulder_filt(i,:)'; left_shoulder_rot1(i,:)=newposition';
    newposition=rot1*left_hip_filt(i,:)'; left_hip_rot1(i,:)=newposition';
...
for i=1:length(left_wrist_rot1(:,1))    % same of elbow, shoulder, hip  
    newposition=rot2*left_wrist_rot1(i,:)'; left_wrist_rot2(i,:)=newposition';
    newposition=rot2*left_elbow_rot1(i,:)'; left_elbow_rot2(i,:)=newposition';
    newposition=rot2*left_shoulder_rot1(i,:)'; left_shoulder_rot2(i,:)=newposition';
    newposition=rot2*left_hip_rot1(i,:)'; left_hip_rot2(i,:)=newposition';
...
% wrist
ones1=ones(length(left_wrist),1);    % same of elbow, shoulder, hip
refsystem_tras_wrist=horzcat(left_wrist_rot2,ones1);

for i=1:length(left_wrist)
    newlocation=translation*refsystem_tras_wrist(i,:)'; newref_filt_wrist(i,:)=newlocation';
end
newref_filt_wrist(:,4)=[];

% elbow
refsystem_tras_elbow=horzcat(left_elbow_rot2,ones1);
for i=1:length(left_elbow)
    newlocation=translation*refsystem_tras_elbow(i,:)'; newref_filt_elbow(i,:)=newlocation';
end
newref_filt_elbow(:,4)=[];

% shoulder
refsystem_tras_shoulder=horzcat(left_shoulder_rot2,ones1);
for i=1:length(left_shoulder)
    newlocation=translation*refsystem_tras_shoulder(i,:)'; newref_filt_shoulder(i,:)=newlocation';
end
newref_filt_shoulder(:,4)=[];

% hip
refsystem_tras_hip=horzcat(left_hip_rot2,ones1);
for i=1:length(left_hip)
